...
long ProjectPointOntoTriangle(double A[3], double B[3], double C[3],
      double Norm[3], double Pos[3], double ProjPt[3], double Bary[3],
      double *Dist)
{
      double M[3][3],Mi[3][3];
      long InPoly,i;

      *Dist = VoV(Pos,Norm) - VoV(A,Norm);
      for(i=0;i<3;i++) {
         ProjPt[i] = Pos[i] - *Dist*Norm[i];
         M[i][0] = A[i];
         M[i][1] = B[i];
         M[i][2] = C[i];
      }
...
/**********************************************************************/
/*  Ref mathworld.wolfram.com.  Search "cubic spline"                 */
/*  Given four nodes (X0,Y0)...(X3,Y3), find y(x), X1 <= x <= X2      */
double CubicSpline(double x, double X[4], double Y[4])
{
      ...
      D1 = (-7.0*z0 + 14.0*z1 -  4.0*z2 + 2.0*z3)/45.0;
      D2 = ( 2.0*z0 -  4.0*z1 + 14.0*z2 - 7.0*z3)/45.0;

      a = Y[1];
      b = D1;
      c = 3.0*(Y[2]-Y[1])-2.0*D1-D2;
      d = -2.0*(Y[2]-Y[1])+D1+D2;
